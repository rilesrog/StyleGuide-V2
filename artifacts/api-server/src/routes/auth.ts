...
// POST /auth/magic-link — generate a 6-digit code and send via Resend
router.post("/auth/magic-link", async (req, res) => {
  ...
  await db.insert(magicLinkTokensTable).values({ email: normalEmail, token, code, expiresAt });
  ...
  const { error: emailError } = await resend.emails.send({
    ...
    to: toAddress,
    subject: `${code} is your StyleSwipe sign-in code`,
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:40px 24px;">
        ...
        <div style="background:#f5f5f5;border-radius:12px;padding:24px;text-align:center;margin-bottom:24px;">
          <span style="font-size:48px;font-weight:700;color:#111;letter-spacing:10px;">${code}</span>
        </div>
...
// POST /auth/magic-verify — exchange a 6-digit code or long token for an app session
router.post("/auth/magic-verify", async (req, res) => {
  const { token, code, email } = req.body as { token?: string; code?: string; email?: string };
  ...
  if (code && email) {
    rows = await db
      ...
      .where(
        and(
          eq(magicLinkTokensTable.email, email.toLowerCase().trim()),
          eq(magicLinkTokensTable.code, code.trim()),
  ...
  const rowEmail = magicRow.email;
  const existing = await db.select().from(usersTable).where(eq(usersTable.email, rowEmail)).limit(1);
  ...
  const name = rowEmail.split("@")[0];
  const [newUser] = await db.insert(usersTable).values({ name, email: rowEmail, token: sessionToken }).returning();
