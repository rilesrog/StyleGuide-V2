...
export const ProductCard = forwardRef<ProductCardRef, ProductCardProps>(
  function ProductCard({ product, onSwipe, isTop, saveLabel = "SAVE", skipLabel = "SKIP" }, ref) {
    const colors = useColors();
    const pan = useRef(new Animated.ValueXY()).current;

    const swipeOut = (liked: boolean, dy = 0) => {
    ...
    useImperativeHandle(ref, () => ({
      triggerSwipe: (liked: boolean) => swipeOut(liked, 0),
    }));
          ...
          <View style={styles.bottomRow}>
            ...
            <View style={styles.tagsRow}>
              {styleTags.map((tag) => (
                <View
                  key={tag}
                  style={[styles.tag, { backgroundColor: colors.primary + "20", borderColor: colors.primary + "40" }]}
                >
...
const styles = StyleSheet.create({
  ...
  saveText: { color: "#4CAF7A", fontSize: 28, fontFamily: "Inter_700Bold", letterSpacing: 2 },
  skipOverlay: {
    position: "absolute",
    top: 48,
    right: 24,
  ...
  skipText: { color: "#E05A45", fontSize: 28, fontFamily: "Inter_700Bold", letterSpacing: 2 },
  infoContainer: {
    position: "absolute",
    bottom: 0,
    left: 0,
  ...
  productName: { fontSize: 17, fontFamily: "Inter_600SemiBold", flex: 1, marginRight: 12 },
  price: { fontSize: 18, fontFamily: "Inter_700Bold" },
  bottomRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  category: { fontSize: 13, fontFamily: "Inter_400Regular", textTransform: "capitalize" },
